...
void TBplot::init_plots() {
	...
	if ( plotkind_ == TBplotbase::kind::sipmHitMap ) {
		for( int i = 0; i < xlow.at(plotkind_).size(); i++ ) {
			...
			tmpHist->SetStats(0); tmpHist->SetTitle(""); tmpHist->GetXaxis()->SetTickLength(0.); tmpHist->GetYaxis()->SetTickLength(0.); tmpHist->GetZaxis()->SetRangeUser(0., hitmapMax); tmpHist->Sumw2();
		...
		colPal->SetStats(0); colPal->SetTitle(""); colPal->GetXaxis()->SetTickLength(0.); colPal->GetYaxis()->SetTickLength(0.); colPal->GetZaxis()->SetRangeUser(0., hitmapMax); colPal->Sumw2();
		for (int i = 1; i <= 10000; i++) colPal->SetBinContent(1, i, (hitmapMax/9999.)*(i-1));
		...
		for( int i = 0; i < 2*xlow.at(plotkind_).size(); i++ ) {
			...
			tmpHist->SetStats(1); tmpHist->SetLineWidth(1); tmpHist->SetTitle(""); tmpHist->Sumw2();
		...
		for( int i = 0; i < 2*xlow.at(plotkind_).size(); i++ ) {
			...
			tmpHist->SetTitle(""); tmpHist->SetStats(0); tmpHist->SetLineWidth(1); tmpHist->GetYaxis()->SetRangeUser(0., 4100.); tmpHist->Sumw2();
			if(i < xlow.at(plotkind_).size()) {
				tmpHist->SetLineColor(kBlue); 
			} else {
				tmpHist->SetLineColor(kRed); 
...
void TBplot::fillADC( TBdetector detid, float adc ) {
	...
	if ( plotkind_ == TBplotbase::kind::hitmap ) {
		...
		if ( isCeren ) {
			if ( Nmodule == 2 && Ntower == 5) plots2D_.at(0)->Fill(column - 0.5, plate - 0.5, adc); // SiPM
		} else {
			if ( Nmodule == 2 && Ntower == 5) plots2D_.at(1)->Fill(column - 0.5, plate - 0.5, adc); // SiPM
		}
	} else if ( plotkind_ == TBplotbase::kind::distribution ) {
		if ( isCeren ) {
			if ( Nmodule == 1 ) {
...
void TBplot::Draw() {
	if ( plotkind_ == TBplotbase::kind::sipmHitMap ) {
		for (int i = 0; i < xlow.at(plotkind_).size(); i++){
			c_->cd(); pads_.at(i)->cd(); plots2D_.at(i)->Draw("col");
		}
	} else if ( plotkind_ == TBplotbase::kind::hitmap ) {
		...
		for (int i = 0; i < xlow.at(plotkind_).size(); i++){
			c_->cd(); pads_.at(i)->cd(); plots1D_.at(i+xlow.at(plotkind_).size())->Draw("Hist"); plots1D_.at(i)->Draw("Hist&sames");
		}
	} 
}
