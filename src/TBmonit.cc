...
TBmonit::TBmonit(const std::vector<std::string> fastList, const std::vector<std::string> waveList, const std::string output)
:fastList_(fastList), waveList_(waveList), outputName_(output)
{
...
void TBmonit::MonitPlots(){
  ...
  TBread readerWave = TBread(); auto anevtWave = TBevt<TBwaveform>();
  TBread readerFast = TBread(); auto anevtFast = TBevt<TBfastmode>();

  std::cout << "[INFO] Waveform : " <<  nevt_wave << "evts | Fast : " <<  nevt_fast << "evts" << std::endl;

  TH2D* comparisonWAVEvsFAST = new TH2D("comparisonWAVEvsFAST", "Wave vs Fast;Wave [ADC];Fast [ADC]", 2*DistBinADC_, -DistMaxADC_, DistMaxADC_, 3*DistBinADC_, -DistMaxADC_, 2*DistMaxADC_); comparisonWAVEvsFAST->SetStats(0);
  ...
  for (unsigned ievt = 0; ievt < nevt_wave; ievt++) {
    ...
    for (int idx = 0; idx < anevtWave.size(); idx++) {
      ...
      for (int jdx = 0; jdx < amid.channelsize(); jdx++) {
        ...
        if ( ievt == 0 ) wavMapWave->fillWaveform(utility.find(cid), achannel.waveform());
      }
    }
    ...
    while(true){
      ...
      if( refevt_fast == nevt_fast -1 ) break;
      ...
      for (int idx = 0; idx < anevtFast.size(); idx++) {
	    ...
	    if (refevt_fast == refevt_wave) break;
    }

    std::cout << refevt_fast << " " << refevt_wave << std::endl;
  ...
  TGraph* WaveAccuTime = new TGraph(WaveXaxis.size(), &(WaveXaxis[0]), &(WaveYaxis__[0]));  WaveAccuTime->SetLineWidth(1); WaveAccuTime->SetLineColor(kBlue+1);
  TGraph* WaveDiffTime = new TGraph(WaveXaxis.size(), &(WaveXaxis[0]), &(WaveYaxis_[0])); WaveDiffTime->SetLineWidth(1); WaveDiffTime->SetLineColor(kBlue+1);
  TGraph* FastAccuTime = new TGraph(FastXaxis.size(), &(FastXaxis[0]), &(FastYaxis__[0]));  FastAccuTime->SetLineWidth(1); FastAccuTime->SetLineColor(kRed+1);
  TGraph* FastDiffTime = new TGraph(FastXaxis.size(), &(FastXaxis[0]), &(FastYaxis_[0])); FastDiffTime->SetLineWidth(1); FastDiffTime->SetLineColor(kRed+1);

  FastAccuTime->SetTitle("Accumulate time"); FastAccuTime->GetXaxis()->SetTitle("Evt"); FastAccuTime->GetYaxis()->SetTitle("ms");
  WaveDiffTime->SetTitle("Passing time per event"); WaveDiffTime->GetXaxis()->SetTitle("Evt"); WaveDiffTime->GetYaxis()->SetTitle("ms");


  TCanvas* c_ = new TCanvas("Time", "Time", 1000, 500);
  c_->cd(); TPad* LPad = new TPad("LPad", "LPad", 0.01, 0.01, 0.49, 0.99); padSet(LPad, 0.12);
  c_->cd(); TPad* RPad = new TPad("RPad", "RPad", 0.51, 0.01, 0.99, 0.99); padSet(RPad, 0.12);

  c_->cd(); LPad->cd(); FastAccuTime->Draw("al"); WaveAccuTime->Draw("l&sames");
  c_->cd(); RPad->cd(); WaveDiffTime->Draw("al"); FastDiffTime->Draw("l&sames");
  c_->SaveAs("Timing.png");

  TCanvas* c_Comp = new TCanvas("comparison","comparison", 500, 500);
  c_Comp->cd(); TPad* CompPad = new TPad("comparison", "comparison", 0.01, 0.01, 0.99, 0.99); padSet(CompPad, 0.12);
  c_Comp->cd(); CompPad->cd(); comparisonWAVEvsFAST->Draw("colz");
  ...
  plotRootFile->cd(); plotRootFile->cd("WaveHitMap");
  for (int i = 0; i < HitMapAccuWave->getPlotSize2D(); i++) HitMapAccuWave->aPlot2D(i)->Write("",TObject::kOverwrite);

  plotRootFile->cd(); plotRootFile->cd("WaveADCDist");
  for (int i = 0; i < disMapAccuWave->getPlotSize1D(); i++) disMapAccuWave->aPlot1D(i)->Write("",TObject::kOverwrite);

  plotRootFile->cd(); plotRootFile->cd("WaveWaveform");
  for (int i = 0; i < wavMapWave->getPlotSize1D(); i++) wavMapWave->aPlot1D(i)->Write("",TObject::kOverwrite);

  plotRootFile->cd(); plotRootFile->cd("FastHitMap");
  for (int i = 0; i < HitMapAccuFast->getPlotSize2D(); i++) HitMapAccuFast->aPlot2D(i)->Write("",TObject::kOverwrite);

  plotRootFile->cd(); plotRootFile->cd("FastADCDist");
  for (int i = 0; i < disMapAccuFast->getPlotSize1D(); i++) disMapAccuFast->aPlot1D(i)->Write("",TObject::kOverwrite);
