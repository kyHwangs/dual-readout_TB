...
int main(int argc, char** argv) {
  ...
  for ( int ievt = 0; ievt < maxEvents; ievt++ ) {
    ...
    float psADC = std::accumulate(psWave_ped.begin() + 240, psWave_ped.begin() + 240 + 140, 0.);
    float muADC = std::accumulate(muWave_ped.begin() + 820, muWave_ped.begin() + 820 + 130, 0.);
  ...
  TFile* rootOutput = new TFile((TString)("/u/user/kyhwang/DRC/Aug2022TB/dev_230307/dual-readout_TB/analysis/AvgTime/Run" + std::to_string(runNum) + "_AuxTimeInfo.root"), "RECREATE");
  rootOutput->cd();
